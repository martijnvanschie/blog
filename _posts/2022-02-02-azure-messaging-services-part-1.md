...
The role of the Event Grid is to orchestrated the distribution of these events in a reliable way to all the indivisual subscribers. This makes it a very suitable service to be used in a publish-subscribe architecture model.
...
> I will dedicate a seperate blog on developing against Azure Event Grid in the future.
...
> It's *important* to note that the topic name should be somehing describing the source because it is often used in filtering events.
...
**Event Grid System Topics** are events that are defined by Azure and are send by specific Azure Resources. They contain a predefined `topic` and can only provide a preset of `eventtype`. They are also registered globally. They are also seperate resources in Azure.
...
Behind the scene Event Grid **Subscriptions** are routing componets that send a http(s) call to another endpoint. But in reality they are not just resources that route event forward to event handlers.
...
As mentioned before Azure Event Grid uses a push pattern in which it *sends* messages from the Event Grid to all subscribers. It uses a one-to-many distribution pattern and each incoming event is discributed to all subscribers that match the event filter.
...
The first thig that is required is for an event handler to create a subscription for a topic. Without any subscriptions the Event Grid has not much to do.
...
If the filter matches the subscription event grid will try to send the message to the event handler untill it receives a acknowledgment from the event handler.
...
One thing you can do to deal with this is to have reliable service act as the event handler. A common pattern for Event Grid subscriptions is actually use a event handler which can store the message and have a client application read from the persistant service.

Below image shows two options for end-to-end "guaranteed delivery".

![](../images/posts/20220202/event-grid-event-handlers-lg.png)

The example shows an architecure pattern where a Queue Storage and Event Hub are subscribed to a Event Grip Topic. The event handler, in this case an Function App, can read from either service. 
...
- For the **Event Hub**, the logic will not update the checkoint or replay the message from the hub
...
For me Event Grid is usefull in scenario's where you want to notify other services of a change that occured using a light weight, easy to deploy, messaging service. Basically a broadcast pattern where the receiver has full control over what to do with the event and when. He could ignore it or process it. Based on the information the receiver can perform additional actions.

If we look at the other two messaging services, they revolve more around exchanging data messages with a more rich payload. These services usually contain business data that need to be consumed by the receiver and can potentially not be obtained from the source besides the message.

information and not want to worry about who consumes the event. Because we don't put any data in the event, except for some basic information.